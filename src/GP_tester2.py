...
[K, Ks, y_train, y_train_var] = Generate_K_Ks(MyMap, MyMap_CPS, Kss)
print(np.shape(Ks))
print(K)
print(y_train)
print(y_train_var)
#Do GP
[y_test, y_test_var]= Calc_GP(K, Ks, Kss, y_train, y_train_var, 2)

y_test_2D = Reshape_1D_to_2D(y_test, MyMap.grid)
y_test_var_2D = Reshape_1D_to_2D(y_test_var, MyMap.grid)


#Plot image
ax1.imshow(MyMap_CPS.grid)
ax2.imshow(y_test_2D,interpolation='none')
